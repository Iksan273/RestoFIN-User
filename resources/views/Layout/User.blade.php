...
<head>
    ...
    <title>Vinautism Art & Resto</title>

    <!-- Favicons-->
    <link rel="shortcut icon" href="{{asset('resto/Logo-Gambar.png')}}" type="image/x-icon">
                            ...
                            <ul id="cart-items" class="cart-items-list">
                                @if (isset($cart) && count($cart) > 0)
                                    @foreach ($cart as $id => $item)
                                        <li data-id="{{ $id }}">
                                            <figure>
                                                <img src="https://admin.bemubaya.com/menu/images/{{ $item['imageUrl'] }}"
